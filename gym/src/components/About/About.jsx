import React from "react";
import BannerImg from "../../assets/about1.jpg";

const About = () => {
  
  return (

    <>
    <span id="about"></span>
    <div className="py-14 dark:bg-dark bg-slate-100 duration-300">
      <div className="container">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 place-items-center">
          <div data-aos="fade-up">
            <img
              src={BannerImg}
              alt=""
              className="sm:scale-125 sm:-translate-x-11 max-h-[300px] drop-shadow-[2px_10px_6px_rgba(0,0,0,0.50)] mx-auto"
            />
          </div>
          <div>
            <div className="space-y-5 sm:p-16 pb-6">
              <div data-aos="zoom-in" className="flex items-center gap-4">
                <div className="text-primary/70 text-7xl ">
                  <h1 className="font-bold">01</h1>
                </div>
                <div>
                  <p className="text-primary">Global Fitness</p>
                  <h1 className="text-2xl sm:text-4xl font-bold">About us</h1>
                </div>
              </div>
              <p data-aos="fade-up" data-aos-delay="300" className=" text-4xl font-bold	">
              ENHANCE YOUR LIFE IN & OUT OF THE GYM
              </p>
              <p data-aos="fade-up" className="leading-8 tracking-wide">
              For over 20 years, <span className="text-primary">BURN GYM</span> has delivered life-changing results to people of all ages and fitness levels.
               The CrossFit formula combines consistent training through constantly varied, functional movement with 
               sound nutrition and community accountability to build a program that never gets old.
              </p>
              <p></p>
                <a href="/#join" data-aos="fade-up" className="button-outline">
                Get Started
                </a>
                
            </div>
          </div>
        </div>
      </div>
    </div>
    </>

    
  );
};

export default About;
